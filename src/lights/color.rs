...
mod test{
    ...
    #[test]
    fn move_to(){
        let current = Color([0,255,0]);
        let target  = Color([0,0,255]);
    ...
    fn dim(){
        ...
        assert_eq!(Color([0,127,0]),current.move_toward(target,0.5));
    }
    #[test]
    fn move_past(){
        let current = Color([0,0,0]);
        let target  = Color([1,2,3]);
        assert_eq!(Color([2,4,6]),current.move_toward(target,2.0));
    }
...
mod serialization{
    ...
    impl Color{
        fn to_int(&self)->i32{
            ...
            for (index,shift) in [(RED_INDEX,RED_SHIFT),(GREEN_INDEX,GREEN_SHIFT),(BLUE_INDEX,BLUE_SHIFT)].iter() {
                val = val | (self.0[*index] as u32) << shift;
            }
    ...
    mod test{
        ...
        #[test]
        fn serialize_black() {
            assert_eq!(serde_json::to_string(&Color([0,0,0])).unwrap(),"0".to_string());
        }
        #[test]
        fn serialize_red() {
            assert_eq!(serde_json::to_string(&Color([15,0,0])).unwrap(),"983040".to_string());
        }
        #[test]
        fn serialize_green() {
            assert_eq!(serde_json::to_string(&Color([0,15,0])).unwrap(),"3840".to_string());
        }
        #[test]
        fn serialize_blue() {
            assert_eq!(serde_json::to_string(&Color([0,0,15])).unwrap(),"15".to_string());
        }
    }
}
